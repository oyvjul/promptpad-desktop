...
export function highlightMarkdown(text: string): string {
  ...
  return lines
    .map((rawLine) => {
      ...
      // Links [text](url)
      line = line.replace(/\[([^\]]*)\]\(([^)]*)\)/g, (_m, text: string, url: string) =>
        ph(
      ...
      line = line.replace(/(\*\*|__)(.+?)\1/g, (_m, marker: string, content: string) =>
        ph(
      ...
      line = line.replace(/\x00(\d+)\x00/g, (_m, i: string) => placeholders[parseInt(i)])
